...
function doPost(e) {
  try {
    ...
    switch (payload.action) {
      ...
      // ---- Restore-from-backup MERGE actions (append-only audit logs) — see the big comment
      // block above RESTORE-FROM-BACKUP MERGE further down this file for why these exist as
      // additive-only merges instead of whole-sheet replace actions.
...
function saveSites(sitesPayload, expectedRev) {
  return withScriptLock_(function () {
    ...
    for (var i = 0; i < sitesPayload.length; i++) {
      ...
      if (!key) {
        var suffix = 1;
        // Checks BOTH the sites that already existed before this save (existingByKey) AND
        // every key already assigned earlier in this same batch (existingByKey gets each new
        // row's key reserved into it right below) — without that second check, two brand-new
        // sites in the same save whose names slugify to the same thing (e.g. "Tripoli" and
        // "TRIPOLI ") could silently end up sharing one key.
        while (existingByKey[newKey] && existingByKey[newKey].displayName !== displayName) {
          newKey = slugify_(displayName) + '_' + (++suffix);
        }
        // Reserve this brand-new key immediately so the very next iteration of this same loop
        // sees it too. Only done for freshly-generated keys — an existing site's own entry in
        // existingByKey is left exactly as-is (readSites_() already has everything it needs).
...
/* =====================================================================
   RESTORE-FROM-BACKUP MERGE ACTIONS — append-only audit logs
   =====================================================================
   WHY "MERGE" AND NOT "REPLACE": Vouchers, Serial ranges, Desert Pass log, and
   the two stock movement logs are append-only audit trails by design (see the
   big comment above REVISIONS_SHEET_NAME) — there is deliberately no "wipe the
   sheet and write the backup's version" action for them, because an old backup
   restored that way would silently erase every voucher/move issued after the
   backup was taken. These five functions instead only ADD whatever the backup
   contains that ISN'T already on the live sheet (matched by serial/id), and
   never touch or remove anything already there. Restoring the same backup
   twice is harmless — the second run just finds everything already present
   and adds nothing.
   ===================================================================== */

// ---- Voucher log ----
// Matches on Serial (the same uniqueness findVoucherRow_ already relies on
// everywhere else in this file). Any voucher whose serial already exists live
// is skipped, never overwritten. Rows are grouped by destination site sheet
// and appended in one batch per sheet for efficiency.
...
// Only ADDS a range row for a site+month key that doesn't exist live yet
// (e.g. a range that predates the current sheet, or was lost). An existing
// key's start/end/current is never touched — overwriting a live "current"
// counter from an old backup could reissue serial numbers already used.
...
// ---- Food Stock movement log ---- (matches on Id)
...
// ---- Stock Site movement log ---- (matches on Category + Site + Id, same
// composite key deleteStockSiteMove_ already uses elsewhere in this file)
...
// ---- Desert Pass log ---- (JSON-rows sheet — matches on 'serial' if present,
// otherwise falls back to an exact full-record match so a record with no
// serial field still can't be duplicated by restoring the same backup twice)
...
//
// UPDATED: previously this always reset "Current" back to "start" whenever Start/End were saved
// from Serial Settings — so simply widening a range's End (or re-saving Start/End unchanged)
// silently rewound the next-serial counter back to the beginning, which could reissue serial
// numbers that were already used. Now the running "Current" position is only touched when the
// caller explicitly passes payload.current (a real, deliberate "set the next serial to this
// number" request) — an ordinary Start/End save leaves whatever the counter currently is alone.
...
// Explicitly sets the Desert Pass "Current" serial counter for a given month to an exact number
// (rather than incrementing it the way nextDesertPassSerial does). Used by an admin action that
// wants to jump the next-issued serial to a specific value — e.g. correcting the counter after a
// manual/offline pass was issued outside the app. Locked for the same read-then-write reasons as
// saveRange/nextDesertPassSerial above.
function setDesertPassSerial(payload) {
  return withScriptLock_(function () {
    ...
    var rangeKey = DP_RANGE_PREFIX + month;
    var sh = getOrCreateSheet_(RANGES_SHEET_NAME, RANGES_HEADERS);
    var data = sh.getDataRange().getValues();
    for (var r = 1; r < data.length; r++) {
...
/* ============ Auto-delete old Voucher PDFs (Drive) — runs monthly, deletes anything
   older than PDF_RETENTION_MONTHS to keep your Drive/Sheet-linked storage from growing
   forever. Files are TRASHED (Drive's own 30-day recycle bin), not permanently destroyed
   outright — so an accidental early cutoff is still recoverable from Drive's trash for a
   month afterward. ============ */

var PDF_RETENTION_MONTHS = 3; // change this number to adjust the cutoff

// Returns the Drive folder used for voucher PDFs (same one uploadPdf() writes into).
function pdfFolder_() {
  var folders = DriveApp.getFoldersByName(PDF_FOLDER_NAME);
  return folders.hasNext() ? folders.next() : null; // null = folder doesn't exist yet, nothing to clean
}

// Does the actual cleanup. Locked so it can't run twice at once (e.g. the monthly trigger
// firing at the same moment someone clicks a manual "run now" button).
...
// Run this ONCE, manually, from the Apps Script editor (pick it from the function dropdown,
// click Run) to install the monthly schedule. Safe to re-run — it clears any existing trigger
// for this job first.
