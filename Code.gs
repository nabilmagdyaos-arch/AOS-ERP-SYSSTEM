...
function doPost(e) {
  try {
    ...
    switch (payload.action) {
      ...
      case 'clearLog':              clearAllVouchers(); resetVoucherSerials_(); return jsonOut({ ok: true });
...
// Every "save the whole list/blob" sheet (Payroll, Passports, Desert Pass Manual + moves,
// Residence, Food Stock, Stock Site, Site Move, Overtime, HR Leave, Outlook, Custom Lists) gets
// an integer revision counter here, bumped every time that sheet is written. A client that last
// read revision N and tries to save gets rejected — not silently overwritten — if the server has
// already moved past N, i.e. someone else saved in the meantime. This intentionally does NOT
// apply to append-only logs (Vouchers, Desert Pass auto log, Food/Stock move logs, Login
// History) since two people appending at once can't lose each other's data to begin with — only
// whole-list "SaveAll" sheets are exposed to that risk.
...
function saveChartOfAccounts(accountsPayload) {
  return withScriptLock_(function () {
    ...
    return { ok: true, accounts: readChartOfAccountsUnlocked_() };
  });
}
