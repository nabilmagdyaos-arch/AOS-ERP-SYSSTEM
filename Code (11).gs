...
// ---------------------------------------------------------------
// Returns the current moment as an ISO-style string in Tripoli local time
// (UTC+2, no DST) instead of new Date().toISOString(), which is always UTC
// and was the cause of the 2-hour-early IssuedAt / login timestamps.
...
// ---------------------------------------------------------------
// Sheets silently auto-converts plain "yyyy-MM-dd" / "HH:mm:ss" text into a
// real Date value the moment it's written to a cell — the sheet still shows
// it looking fine, but reading it back with plain String(cellValue) returns
// Date.prototype.toString() (e.g. "Thu Jul 16 2026 00:00:00 GMT+0200"), which
// no longer matches the "yyyy-MM-dd" / "HH:mm:ss" strings the app filters and
// sorts by. This reformats it back into the exact string format the app
// expects, whether the cell ended up as a real Date or was left as plain text.
